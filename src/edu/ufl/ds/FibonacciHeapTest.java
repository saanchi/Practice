...
public class FibonacciHeapTest {
	...
	@Test
    public void testPrimcase(){
    	FibonacciHeap heap = new FibonacciHeap();
        assertTrue(heap.isEmpty());
        assertEquals(0, heap.size());
        Map<Integer, FibonacciHeap.Node> entries =  new HashMap<Integer, FibonacciHeap.Node>();
        entries.put( 100, heap.insert(100, 0));
        for (int ii = 101; ii < 105; ii++) {
        	// Insert with same values
            Integer it = new Integer( Integer.MAX_VALUE);
            entries.put(ii, heap.insert(ii, it));
        }
        FibonacciHeap.Node entry = entries.get(100);
        assertEquals( entry,  heap.min());
        // Deque it
        assertEquals(100, heap.removeMin());
        // Decrease key
        entry = entries.get(101);
        heap.decreaseKey( entry, 500);	
        assertEquals( entry,  heap.min());
        // Deque it
        assertEquals(101, heap.removeMin());
        // Decrease key
        entry = entries.get(102);
        heap.decreaseKey( entry, 400);	
        assertEquals( entry,  heap.min());
        // Deque it
        assertEquals(102, heap.removeMin());
        // Decrease key
        entry = entries.get(103);
        heap.decreaseKey( entry, 300);	
        assertEquals( entry,  heap.min());
        // Deque it
        assertEquals(103, heap.removeMin());
     // Decrease key
        entry = entries.get(104);
        heap.decreaseKey( entry, 490);	
        assertEquals( entry,  heap.min());
        // Deque it
        assertEquals(104, heap.removeMin());
